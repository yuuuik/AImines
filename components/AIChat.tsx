...
import { useGameStore, AIMessage, AIAgent } from '@/store/gameStore';
...
export default function AIChat() {
  const { aiMessages, isTyping, phase, mineCount, revealedCount, currentMult, selectedAgent, setSelectedAgent } = useGameStore();
    ...
    <div className="flex flex-col h-full" style={{ background: '#141415' }}>
      ...
      <div className="flex items-center gap-2.5 px-3 py-2.5 border-b border-white/5 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-white/30 text-[9px] mt-0.5">AI Аналитик · Mines</p>
        </div>
        <div
            ...
            <div className="rounded-xl border p-3 space-y-2 bg-gradient-to-br from-red-500/8 to-transparent border-red-500/20">
              ...
              <div className="flex gap-2">
                <div className="flex-1 text-center">
                  <p className="text-[9px] text-white/30">Мин</p>
                  <p className="text-red-400 font-bold text-sm">💣{mineCount}</p>
                </div>
                <div className="flex-1 text-center">
                  <p className="text-[9px] text-white/30">Открыто</p>
                  <p className="text-blue-400 font-bold text-sm">{revealedCount}/{25 - mineCount}</p>
                </div>
                <div className="flex-1 text-center">
                  <p className="text-[9px] text-white/30">Множитель</p>
                  <p className="text-green-400 font-bold text-sm">×{revealedCount > 0 ? currentMult.toFixed(2) : '1.00'}</p>
                </div>
              </div>
            </div>
          </motion.div>
        ...
        {aiMessages.length === 0 && !isTyping && (
          ...
          >
            ...
            <div>
              ...
              <p className="text-white/20 text-xs mt-1">Выберите количество мин<br />и нажмите СТАВКА</p>
            </div>
          </motion.div>
        )}
        ...
        <AnimatePresence initial={false}>
          {aiMessages.map(m => (
            <motion.div
              ...
              className={`flex gap-2 items-start rounded-xl px-3 py-2.5 border text-xs leading-snug ${bg(m.type)}`}
            >
              <MsgIcon type={m.type} />
              <span className={tc(m.type)}>{m.text}</span>
            </motion.div>
          ))}
        </AnimatePresence>
