...
function OnlineCounter() {
  ...
  useEffect(() => {
    setCount(1080 + Math.floor(Math.random() * 480));
    const t = setInterval(() => {
      setCount(c => Math.max(900, Math.min(2100, c + Math.floor(Math.random() * 13) - 6)));
    }, 2200 + Math.floor(Math.random() * 1000));
    return () => clearInterval(t);
  }, []);
  return (
    <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg mb-6" style={{ background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.12)' }}>
      <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse shrink-0" />
      <span className="text-green-400 text-[11px] font-bold tabular-nums">{count.toLocaleString('ru-RU')}</span>
      <span className="text-white/30 text-[11px]">онлайн</span>
    </div>
  );
}
  ...
  return (
    <aside className="flex flex-col h-full w-full px-5 py-8 gap-2 select-none" style={{ background: '#1c1c1e', borderRight: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div className="flex items-center gap-3 px-1 mb-8">
        ...
        <span className="text-white font-black text-lg tracking-wider leading-tight uppercase">MINES</span>
      </div>
